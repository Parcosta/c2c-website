...
const customDomainName =
  app.node.tryGetContext("customDomainName") ?? process.env.CUSTOM_DOMAIN_NAME;
...
new AmplifyStack(app, "AmplifyStack", {
  ...
  parameters: {
    ...
    sanityApiToken:
      app.node.tryGetContext("SANITY_API_TOKEN_PARAM") ?? "/c2c-website/SANITY_API_TOKEN",
    sanityWebhookSecret:
      app.node.tryGetContext("SANITY_WEBHOOK_SECRET_PARAM") ?? "/c2c-website/SANITY_WEBHOOK_SECRET",
    resendApiKey: app.node.tryGetContext("RESEND_API_KEY_PARAM") ?? "/c2c-website/RESEND_API_KEY",
    contactFormTo:
