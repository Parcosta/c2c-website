...
async function fetchUiContentFromSanity(locale: Locale): Promise<UiContentMap> {
  ...
  try {
    ...
    const results = await getClient().fetch<Array<{ key: string; text: string }>>(query, {
      locale
    });
...
export async function getTranslation(locale: Locale) {
  ...
  return function t(key: string): string {
    ...
    // Use Sanity value if available, otherwise fallback to local translations, then key
    return value ?? getFallbackTranslation(locale, key) ?? key;
  };
}
...
// These should match the translations in src/i18n/index.ts
const fallbackTranslations: Record<Locale, Record<string, string>> = {
  en: {
    ...
    "home.metaDescription": "Live modular techno & DJ. Music, shows, and releases by Coast2Coast (C2C).",
    ...
    "gallery.lightboxInstructions": "Use arrow keys to navigate between images. Press Escape to close.",
    ...
    "currentWork.fallbackDescription": "I'm currently working on the next piece in my portfolio. Check back soon for updates.",
    "audio.play": "Play",
    "audio.pause": "Pause",
    ...
    "error.subtitle": "Try again in a moment. If the problem persists, return home and retry from there.",
    ...
    "cookieConsent.description": "We use cookies to enhance your browsing experience and analyze our traffic.",
    ...
    "cookieConsent.analyticsDescription": "Helps us understand how visitors interact with our website.",
  ...
  es: {
    ...
    "home.metaDescription": "Techno modular en vivo y DJ. Música, shows y lanzamientos de Coast2Coast (C2C).",
    ...
    "gallery.lightboxInstructions": "Usa las flechas para navegar entre imágenes. Presiona Escape para cerrar.",
    ...
    "currentWork.fallbackDescription": "Actualmente estoy trabajando en la siguiente pieza de mi portafolio. Vuelve pronto para ver actualizaciones.",
    ...
    "error.subtitle": "Inténtalo de nuevo en un momento. Si el problema persiste, regresa al inicio e intenta desde allí.",
    ...
    "cookieConsent.description": "Usamos cookies para mejorar tu experiencia de navegación y analizar nuestro tráfico.",
    ...
    "cookieConsent.necessaryDescription": "Esenciales para que el sitio web funcione correctamente.",
    "cookieConsent.analyticsLabel": "Analíticas",
    "cookieConsent.analyticsDescription": "Nos ayuda a entender cómo los visitantes interactúan con nuestro sitio.",
